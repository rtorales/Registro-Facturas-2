<template>
  <div>
    ...
    <v-btn color="primary mb-3 mx-3" @click="getUsersCSV"> Download CSV </v-btn>
    ...
    >
      ...
      <template v-slot:item.Contribuyente="{ item }">
        <span
          v-for="(i, idx) in dataFormatter.contribuyentesManyListFormatter(
            item.Contribuyente,
          )"
          :key="idx + item.Contribuyente"
          class="d-block"
        >
          {{ i }}
        </span>
